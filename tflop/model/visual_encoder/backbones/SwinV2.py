...
class WindowAttention(nn.Module):
    ...
    def forward(
        self: "WindowAttention", x: torch.Tensor, mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        Args:
        ...
        if self.q_bias is not None:
            ...
            qkv = F.linear(input=x, weight=self.qkv.weight) + qkv_bias
        else:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        pretrained_window_size: Union[int, Tuple[int, int]] = 0,
    ) -> None:
        super().__init__()
        self.dim = dim
    ...
    def _calc_window_shift(
        ...
        target_shift_size: Union[int, Tuple[int, int]],
    ...
    def _attn(self: "SwinTransformerBlock", x: torch.Tensor) -> torch.Tensor:
        H, W = self.input_resolution
        B, L, C = x.shape
    ...
    def forward(self: "SwinTransformerBlock", x: torch.Tensor) -> torch.Tensor:
...
class PatchMerging(nn.Module):
    ...
    def forward(self: "PatchMerging", x: torch.Tensor) -> torch.Tensor:
        """
        x: B, H*W, C
        """
        H, W = self.input_resolution
...
class BasicLayer(nn.Module):
    ...
    def forward(self: "BasicLayer", x: torch.Tensor) -> torch.Tensor:
    ...
    def _init_respostnorm(self: "BasicLayer") -> None:
        for blk in self.blocks:
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(
        ...
        pretrained_window_sizes: Tuple[Union[int, Tuple[int, int]], ...] = (0, 0, 0, 0),
        **kwargs: Any,
    ) -> None:
        super().__init__()
    ...
    def _init_weights(self: "SwinTransformerV2", m: nn.Module) -> None:
    ...
    @torch.jit.ignore
    def no_weight_decay(self: "SwinTransformerV2") -> Set[str]:
    ...
    @torch.jit.ignore
    def group_matcher(
        self: "SwinTransformerV2", coarse: bool = False
    ) -> Dict[str, Any]:
        return dict(
    ...
    @torch.jit.ignore
    def set_grad_checkpointing(self: "SwinTransformerV2", enable: bool = True) -> None:
        for layer in self.layers:
            layer.grad_checkpointing = enable

    @torch.jit.ignore
    def get_classifier(self: "SwinTransformerV2") -> nn.Module:
        return self.head

    def reset_classifier(
        self: "SwinTransformerV2",
        num_classes: int,
        global_pool: Union[str, None] = None,
    ) -> None:
        self.num_classes = num_classes
    ...
    def forward_features(self: "SwinTransformerV2", x: torch.Tensor) -> torch.Tensor:
    ...
    def forward_head(
        self: "SwinTransformerV2", x: torch.Tensor, pre_logits: bool = False
    ) -> torch.Tensor:
        if self.global_pool == "avg":
            x = x.mean(dim=1)
        return x if pre_logits else self.head(x)

    def forward(self: "SwinTransformerV2", x: torch.Tensor) -> torch.Tensor:
...
def checkpoint_filter_fn(
    state_dict: Mapping[str, Any], model: nn.Module
) -> Mapping[str, Any]:
    out_dict = {}
...
def _create_swin_transformer_v2(
    variant: str, pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
...
@register_model
def swinv2_base_window12_192_22k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
...
@register_model
def swinv2_base_window12to16_192to256_22kft1k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
...
@register_model
def swinv2_base_window12to24_192to384_22kft1k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
...
@register_model
def swinv2_large_window12_192_22k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
...
@register_model
def swinv2_large_window12to16_192to256_22kft1k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
...
@register_model
def swinv2_large_window12to24_192to384_22kft1k(
    pretrained: bool = False, **kwargs: Any
) -> SwinTransformerV2:
    """ """
    model_kwargs = dict(
